...
export class ProjectQuery extends Query<ProjectView, ProjectViewRepo, ProjectView[]> {
	...
	query(): ProjectView[] {
		...
		) as PersistableEvent<DrawingEvent>[]
		const projects = events
			.filter((event) => event.eventType === "NewProjectEvent")
			.map((event) => new ProjectView((event.payload as unknown as NewProjectEvent).name))
