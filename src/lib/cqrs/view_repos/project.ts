...
import { persistableEventsToDrawingEvents, type DrawingEvents } from "../../../types/commands-events";
...
export class ProjectViewRepo extends ViewRepository<DrawingEvents, ProjectView> {
	...
	async load(aggregateId: string): Promise<ProjectView> {
		...
		const project: PersistableEvent[] = JSON.parse(localStorage.getItem("events") || "[]")
